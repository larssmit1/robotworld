...
class Matrix
{
	public:
		 ...
		 */
		std::tuple< Matrix< T, M ,N >, Matrix< T, N ,N >, Matrix< T, N ,N > > svd() const;
		//@}
		/**
		 * @name Other methods
		 */
	...
	private:
		/**
		 * @return the appropriate row number that can be used for the current pivotColumn. This is the row with the maximum absolute value in the pivotColumn
		 */
		std::size_t getMaxPivotRow( const Matrix< T, M, N >& aMatrix,
									std::size_t pivotColumn) const;
		/**
		 *
		 */
		T getLineariseFactor(	const Matrix< T, M, N >& aMatrix,
